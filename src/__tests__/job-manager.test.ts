...
describe('JobManager', () => {
  ...
  describe('processJob', () => {
    ...
    it('should process a job and create all required files', async () => {
      ...
      const jobIndexContent = JSON.parse(fs.readFileSync(path.join(jobDir, 'job-index.json'), 'utf-8'));
      expect(jobIndexContent.title).toBe('Software Engineer');
      expect(jobIndexContent.shortcode).toBe('SE001');

      const stagesJsonContent = JSON.parse(fs.readFileSync(path.join(jobDir, 'stages.json'), 'utf-8'));
      expect(stagesJsonContent.stages).toHaveLength(3);
      expect(stagesJsonContent.stages[0].name).toBe('Sourced');

      const stagesMarkdownContent = fs.readFileSync(path.join(jobDir, 'stages.md'), 'utf-8');
      expect(stagesMarkdownContent).toContain('# Software Engineer - Recruitment Stages');
      ...
      expect(stagesMarkdownContent).toContain('| 3 | Phone Interview | phone_interview |');
    ...
    it('should handle stage fetching errors gracefully', async () => {
      ...
      expect(console.error).toHaveBeenCalledWith('  Failed to process stages for Software Engineer: API Error');
    });
  });
